...
public class MaskHero : Hero
{
    ...
    public void StartIonStormLoop(SkillDataSO data) => StartCoroutine(IonStormLoop(data));

    // 이온폭풍: 최종 승급 즉시 생성 후, data.loopInterval초 쿨마다 재생성
    IEnumerator IonStormLoop(SkillDataSO data)
    {
        while (true)
        {
            SpawnIonStorm(data);
    ...
    void SpawnIonStorm(SkillDataSO data)
    {
        ...
        IonStorm.Spawn(ionStormVfxPrefab, pos, stats.attackDamage, data.lifetime);
    }
}
